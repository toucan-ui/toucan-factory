import { Tabs, TabList, Tab, TabPanel, Text, Heading, Box, Link, Grid, Section } from '@toucan-ui/core';
...
const categories = PATTERN_CATEGORIES.map((cat) => ({
  ...
  patterns: PATTERNS
    ...
    .map((p) => ({ name: p.name, slug: p.slug, description: p.description })),
        ...
        {categories.map((cat) => (
          <TabPanel key={cat.value} value={cat.value}>
            <Grid columns="auto" gap={4} minItemSize="var(--layout-50)">
              {cat.patterns.map((p) => (
                <Link key={p.slug} href={`/patterns/${p.slug}`} variant="standalone" className="docs-link-card">
                  <Box padding="md" radius="md" elevation={1}>
                    <Heading level={4}>{p.name}</Heading>
                    <Text muted size="sm">{p.description}</Text>
                  </Box>
                </Link>
              ))}
            </Grid>
