...
const RAW_COLORS = [
  ...
  { name: 'Neutral', tokens: ['neutral.0', 'neutral.50', 'neutral.100', 'neutral.200', 'neutral.300', 'neutral.400', 'neutral.500', 'neutral.600', 'neutral.700', 'neutral.800', 'neutral.900', 'neutral.950'] },
...
const RAW_SCALE = [
  '0 (0px)', '0-25 (1px)', '0-5 (2px)', '1 (4px)', '2 (8px)', '3 (12px)', '4 (16px)',
  '5 (20px)', '6 (24px)', '7 (28px)', '8 (32px)', '9 (36px)', '10 (40px)',
  '11 (44px)', '12 (48px)', '16 (64px)',
];

const RAW_LAYOUT = [
  '20 (80px)', '40 (160px)', '45 (180px)',
  '50 (200px)', '60 (240px)', '70 (280px)', '80 (320px)', '120 (480px)', '140 (560px)',
  '192 (768px)', '240 (960px)', '256 (1024px)', '300 (1200px)', '320 (1280px)',
];

const ALIAS_COLORS = [
  { name: 'Primary', tokens: ['color.primary', 'color.primary-hover', 'color.primary-active', 'color.on-primary'] },
  { name: 'Surface', tokens: ['color.surface.default', 'color.surface.muted', 'color.surface.raised'] },
  ...
  { name: 'Semantic', tokens: ['color.danger', 'color.danger-surface', 'color.success', 'color.success-surface', 'color.warning', 'color.warning-surface', 'color.info', 'color.info-surface'] },
  { name: 'State', tokens: ['color.disabled', 'color.on-disabled', 'color.backdrop'] },
];

const ALIAS_TYPOGRAPHY = [
  'text.font-family', 'text.font-family-mono',
  'heading.font-weight', 'heading.letter-spacing',
  'display.font-family', 'display.font-weight', 'display.letter-spacing', 'display.line-height',
  'body.font-weight', 'body.font-size', 'body.line-height', 'body.letter-spacing',
];

const SYSTEM_COMPONENTS = [
  'Button', 'Input', 'Textarea', 'Select', 'Checkbox', 'Toggle', 'Radio',
  'Surface', 'Text', 'Badge', 'Alert', 'Avatar', 'Separator', 'Skeleton',
  'Progress', 'Breadcrumb', 'Table', 'Tabs', 'Tooltip', 'Popover',
          ...
          <TabPanel value="raw">
            <Grid gap={8}>
                  ...
                  <Grid key={group.name} gap={2}>
                    ...
                    {group.tokens.map((token) => (
                      <Flex key={token} row gap={2} align="center">
                        ...
                        />
                        <Text as="span" size="sm"><code>--color-{token.replace('.', '-')}</code></Text>
                      </Flex>
                    ))}
                  </Grid>
                ))}
              ...
              <Grid gap={4}>
                ...
                <Text muted>Unified spatial scale for spacing, sizing, and gaps. Key × 4 = pixels.</Text>
                <Grid gap={2}>
                  {RAW_SCALE.map((s) => {
                    const key = s.split(' ')[0];
                    return <FieldRow key={key} label={`--scale-${key}`} value={s.match(/\(.*\)/)?.[0] || ''} mono />;
                  })}
                </Grid>
              </Grid>
              ...
              <Grid gap={4}>
                ...
                <Text muted>Layout-scale tokens for larger dimensions — widths, max-widths, breakpoints. Key × 4 = pixels.</Text>
                <Grid gap={2}>
                  {RAW_LAYOUT.map((s) => {
                    const key = s.split(' ')[0];
                    return <FieldRow key={key} label={`--layout-${key}`} value={s.match(/\(.*\)/)?.[0] || ''} mono />;
                  })}
                </Grid>
              </Grid>
              ...
              <Grid gap={4}>
                ...
                <Text muted>Typography (11 font sizes, 4 weights, 3 families, 4 line heights), Radius (5 steps), Shadow (4 levels), Border widths, Z-index, Opacity, Motion, and Letter spacing.</Text>
              </Grid>
            </Grid>
          </TabPanel>
          ...
          <TabPanel value="alias">
            <Grid gap={8}>
              <Grid gap={4}>
                ...
                <Text muted>Semantic color roles that reference raw values. Themes remap these.</Text>
                  ...
                  <Grid key={group.name} gap={2}>
                    ...
                    {group.tokens.map((token) => (
                      <Flex key={token} row gap={2} align="center">
                        ...
                        />
                        <Text as="span" size="sm"><code>--{token.replace(/\./g, '-')}</code></Text>
                      </Flex>
                    ))}
                  </Grid>
                ))}
              ...
              <Grid gap={4}>
                ...
                <Text muted>Font family, weight, and sizing tokens for text, headings, and display text.</Text>
                <Grid gap={2}>
                  {ALIAS_TYPOGRAPHY.map((token) => (
                    <FieldRow key={token} label={`--${token.replace(/\./g, '-')}`} value="(see computed value)" mono />
                  ))}
                </Grid>
              </Grid>
            </Grid>
              ...
              <Grid gap={4}>
                ...
                <Flex row wrap gap={3}>
                  {SYSTEM_COMPONENTS.map((comp) => (
                    <Badge key={comp} variant="neutral">{comp}</Badge>
                  ))}
                </Flex>
              </Grid>
