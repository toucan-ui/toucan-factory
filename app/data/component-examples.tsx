...
function PaginationDemo() {
  ...
  return (
    <Grid gap={3}>
      <Text size="sm" muted>Page {page} of 20</Text>
...
function ToastDemoInner() {
  ...
  return (
    <Flex row wrap gap={3}>
      <Button variant="secondary" onClick={() => toast({ message: 'Info toast', variant: 'info' })}>Info</Button>
      <Button variant="secondary" onClick={() => toast({ message: 'Saved successfully', variant: 'success' })}>Success</Button>
      <Button variant="secondary" onClick={() => toast({ message: 'Check your input', variant: 'warning' })}>Warning</Button>
      <Button variant="secondary" onClick={() => toast({ message: 'Something went wrong', variant: 'danger' })}>Danger</Button>
    </Flex>
  );
}
      ...
      <Flex row gap={3}>
        <FlexItem shrink={false}>
          <Box padding="md" radius="sm" elevation={0}>
            <Text as="span" size="sm">Fixed (no shrink)</Text>
          </Box>
        </FlexItem>
        <FlexItem grow>
          <Box padding="md" radius="sm" elevation={0}>
            <Text as="span" size="sm">Grows to fill space</Text>
          </Box>
        </FlexItem>
        <FlexItem>
          <Box padding="md" radius="sm" elevation={0}>
            <Text as="span" size="sm">Default</Text>
          </Box>
        </FlexItem>
      </Flex>
    ),
    ...
    demo: (
      <Grid gap={4}>
        <Grid columns={3} gap={3}>
          <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>1</Text>
          </div>
          <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>2</Text>
          </div>
          <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>3</Text>
          </div>
        </Grid>
        <Grid columns="auto" gap={3} minItemSize="96px">
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Auto 1</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Auto 2</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Auto 3</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Auto 4</Text>
          </div>
        </Grid>
        <Grid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Responsive 1</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Responsive 2</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">Responsive 3</Text>
          </div>
        </Grid>
      </Grid>
    ),
    ...
    demo: (
      <>
        <Grid columns={4} gap={3}>
          <GridItem span={2}>
            <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>Span 2</Text>
            </div>
          </GridItem>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">1 col</Text>
          </div>
          <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
            <Text as="span" size="sm">1 col</Text>
          </div>
          ...
          <GridItem span={3}>
            <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>Span 3</Text>
            </div>
          </GridItem>
          <GridItem span={{ base: 4, md: 2 }}>
            <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm">Full on mobile, half on md+</Text>
            </div>
          </GridItem>
          <GridItem span={{ base: 4, md: 2 }}>
            <div style={{ background: 'var(--color-border-default)', padding: 'var(--scale-4)', borderRadius: 'var(--radius-sm)' }}>
  ...
  page: {
    demo: (
      <Box padding="md" radius="md" elevation={1}>
        <Text as="span" size="sm">Page provides min-height: 100vh with themed background and text colour. Typically the outermost wrapper in your app.</Text>
      </Box>
    ),
    code: `<Page>
  <NavBar ... />
    ...
    demo: (
      <Grid gap={0}>
        <Section background="default" padding="md">
          <Text as="span" size="sm">Section — default background, medium padding</Text>
        </Section>
        <Section background="muted" padding="lg">
          <Text as="span" size="sm">Section — muted background, large padding</Text>
        </Section>
        <Section background="default" padding="sm">
          <Text as="span" size="sm">Section — default background, small padding</Text>
        </Section>
      </Grid>
    ),
    ...
    demo: (
      <Grid gap={3}>
        <div style={{ background: 'var(--color-surface-muted)' }}>
          <Wrapper size="xs">
            <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>xs — 480px</Text>
            </div>
          </Wrapper>
        </div>
        <div style={{ background: 'var(--color-surface-muted)' }}>
          <Wrapper size="sm">
            <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>sm — 560px</Text>
            </div>
          </Wrapper>
        </div>
        <div style={{ background: 'var(--color-surface-muted)' }}>
          <Wrapper size="md">
            <div style={{ background: 'var(--color-primary)', padding: 'var(--scale-3)', borderRadius: 'var(--radius-sm)' }}>
              <Text as="span" size="sm" style={{ color: 'var(--color-on-primary)' }}>md — 768px</Text>
            </div>
          </Wrapper>
        </div>
      </Grid>
  ...
  box: {
    demo: (
      <Grid columns="auto" gap={3} minItemSize="128px">
        <Box elevation={0} padding="md" radius="sm"><Text as="span" size="sm">Elevation 0</Text></Box>
        <Box elevation={1} padding="md" radius="md"><Text as="span" size="sm">Elevation 1</Text></Box>
        <Box elevation={2} padding="md" radius="md"><Text as="span" size="sm">Elevation 2</Text></Box>
        <Box elevation={3} padding="lg" radius="lg"><Text as="span" size="sm">Elevation 3</Text></Box>
      </Grid>
    ),
  ...
  tooltip: {
    demo: (
      <Flex row wrap gap={3}>
        ...
        <Tooltip content={<><Heading level={4}>Rich tooltip</Heading><Text size="sm">Tooltips support structured content.</Text></>} anchor="bottom" maxWidth="256px">
...
>
  ...
  breadcrumb: {
    demo: (
      <Breadcrumb>
        <BreadcrumbItem><Link href="/" variant="inline">Home</Link></BreadcrumbItem>
        <BreadcrumbItem><Link href="/components" variant="inline">Components</Link></BreadcrumbItem>
  ...
  link: {
    demo: (
      <Grid gap={3}>
        <Link href="#" variant="inline">Inline link with underline</Link>
...
<Link href="https://example.com" external>External link</Link>`,
  ...
  icon: {
    demo: (
      <Flex row wrap gap={3}>
        <Icon size="sm">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="8" cy="8" r="6" /><path d="M8 5.5v5M5.5 8h5" /></svg>
        </Icon>
        <Icon size="md">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="8" cy="8" r="6" /><path d="M8 5.5v5M5.5 8h5" /></svg>
        </Icon>
      ...
      <Grid gap={3} className="docs-demo-constrain">
        <Accordion defaultValue="features">
          <AccordionItem value="features">
            ...
            <AccordionPanel>
              <Text size="sm">Token-driven design system with accessible primitives, compound components, and theme support.</Text>
            </AccordionPanel>
          </AccordionItem>
          ...
          <AccordionItem value="usage">
            ...
            <AccordionPanel>
              <Text size="sm">Import components from @toucan-ui/core and tokens CSS from @toucan-ui/tokens/css.</Text>
            </AccordionPanel>
          </AccordionItem>
        </Accordion>
      </Grid>
