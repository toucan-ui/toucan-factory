...
import type { ConfigState, ConfigAction, ColourScale } from './types';
...
function configReducer(state: ConfigState, action: ConfigAction): ConfigState {
  switch (action.type) {
    ...
    case 'UPDATE_SCALE': {
      ...
      const newState = {
        ...state,
        palette: state.palette.map((s) => (s.id === action.id ? { ...s, ...action.scale } : s)),
      };
    ...
    case 'LOAD_STATE': {
      ...
      const {
        ...
        opacity: _op,
        ...rest
      } = action.state as ConfigState & Record<string, unknown>;
      ...
      const OPACITY_MIGRATION: Record<string, string> = {
        transparent: '0',
        'translucent-lighter': '30',
        'translucent-light': '40',
        translucent: '50',
        'translucent-medium': '60',
        'translucent-strong': '70',
        opaque: '100',
      };
      const rawOverrides =
        (action.state as ConfigState & Record<string, unknown>).componentOverrides ?? {};
      const migratedOverrides: Record<string, Record<string, string>> = {};
      for (const [comp, overrides] of Object.entries(
        rawOverrides as Record<string, Record<string, string>>,
      )) {
      ...
      const loadedAliases = (rest.typographyAliases ??
        defaults.typographyAliases) as unknown as Record<string, unknown>;
      const {
        bodySize: _bs,
        h1Size: _h1s,
        h2Size: _h2s,
      ...
      const loadedTypography = (rest.typography ?? defaults.typography) as unknown as Record<
        string,
        unknown
      >;
      ...
      return {
        ...
        typographyAliases: {
          ...defaults.typographyAliases,
          ...cleanAliases,
        } as unknown as ConfigState['typographyAliases'],
