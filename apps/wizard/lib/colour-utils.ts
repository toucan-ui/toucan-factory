...
function xyzToOklab(x: number, y: number, z: number): [number, number, number] {
  ...
  return [
    0.2104542553 * l_c + 0.793617785 * m_c - 0.0040720468 * s_c,
    1.9779984951 * l_c - 2.428592205 * m_c + 0.4505937099 * s_c,
    0.0259040371 * l_c + 0.7827717662 * m_c - 0.808675766 * s_c,
  ];
}
...
export function generateScale(baseHex: string): Array<{ step: number; hex: string }> {
  ...
  return SHADE_STEPS.map((step) => {
    ...
    const chromaMin = step < 500 ? chromaPeak * chromaLightMul : chromaPeak * chromaDarkMul;
