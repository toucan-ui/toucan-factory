...
const COMPONENT_OPTIONS = ALL_COMPONENT_KEYS.sort((a, b) => a.localeCompare(b)).map((key) => ({
  value: key,
...
function renderColourOption(option: SelectOption) {
  ...
  return (
    <span className="configurator-colour-option">
      <span className="configurator-colour-option-swatch" style={{ backgroundColor: hex }} />
      {option.label}
    </span>
  );
}
...
function TokenField({ def, value, onChange, onClear }: TokenFieldProps) {
  ...
  return (
    <Flex row gap={3} align="center">
      <Text
        size="sm"
        weight={isOverridden ? 'semibold' : 'regular'}
        className="configurator-token-label"
      >
        {def.token}
      </Text>
