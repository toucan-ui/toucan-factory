...
function DashboardContent() {
  ...
  const handleSave = () => {
    ...
    toast({
      ...
      description: 'All changes applied to the dashboard.',
      variant: 'success',
    });
  };
              ...
              <Flex gap={2}>
                ...
                {overBudget && (
                  <Alert variant="danger">
                    <Text size="sm">
                      Budget exceeds revenue — consider reducing add-ons or increasing ticket price.
                    </Text>
                  </Alert>
                )}
              </Flex>
        ...
        <Flex gap={6}>
          ...
          <FormSection title="Capacity" description="Set expected attendance and duration.">
            <Flex gap={3}>
              <Flex gap={1}>
                <Text size="sm" muted>
                  Attendees: {config.attendees.toLocaleString()}
                </Text>
                <Slider
                  label="Attendees"
              ...
              <Flex gap={1}>
                <Text size="sm" muted>
                  Ticket price: {fmt(config.ticketPrice)}
                </Text>
                <Slider
                  label="Ticket price"
          ...
          <FormSection title="Venue & Catering" description="Choose venue size and food options.">
            <Select
              label="Venue size"
              options={venueSizeOptions}
              value={config.venueSize}
              onChange={(v) => update('venueSize', v)}
            />
            <Select
              label="Catering tier"
              options={cateringTierOptions}
              value={config.cateringTier}
              onChange={(v) => update('cateringTier', v)}
            />
            <Select
              label="Timezone"
          ...
          <FormSection title="Add-ons" description="Optional services to include.">
            <Checkbox
              label="Meal tickets"
              checked={config.mealTickets}
              onChange={(e) => update('mealTickets', e.target.checked)}
            />
            <Checkbox
              label="VIP lounge"
              checked={config.vipLounge}
              onChange={(e) => update('vipLounge', e.target.checked)}
            />
            <Checkbox
              label="Live streaming"
              checked={config.liveStreaming}
              onChange={(e) => update('liveStreaming', e.target.checked)}
            />
            <Checkbox
              label="Swag bags"
              checked={config.swagBags}
              onChange={(e) => update('swagBags', e.target.checked)}
            />
            <Checkbox
              label="Childcare"
              checked={config.childcare}
              onChange={(e) => update('childcare', e.target.checked)}
            />
          </FormSection>
          ...
          <FormSection title="Settings" description="Event visibility and notifications.">
            <Toggle
              label="Public event"
              checked={config.publicEvent}
              onChange={(v) => update('publicEvent', v)}
            />
            <Toggle
